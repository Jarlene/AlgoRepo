...
class CriteoDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, dataset_path=None, cache_path='.criteo', rebuild_cache=False, min_threshold=10):
        ...
        with self.env.begin(write=False) as txn:
            ...
            self.field_dims = np.frombuffer(
                txn.get(b'field_dims'), dtype=np.int64)

    def __getitem__(self, index):
        with self.env.begin(write=False) as txn:
            np_array = np.frombuffer(
                txn.get(struct.pack('>I', index)), dtype=np.int64)

        return {'x': np_array[1:], 'y': np_array[0]}
    ...
    def __build_cache(self, path, cache_path):
        ...
        with lmdb.open(cache_path, map_size=int(1e11)) as env:
            field_dims = np.zeros(self.NUM_FEATS, dtype=np.int64)
    ...
    def __get_feat_mapper(self, path):
        ...
        with open(path) as f:
            ...
            pbar.set_description(
                'Create criteo dataset cache: counting features')
            for line in pbar:
                values = line.rstrip('\n').split('\t')
    ...
    def __yield_buffer(self, path, feat_mapper, defaults, buffer_size=int(1e5)):
        ...
        with open(path) as f:
            ...
            for line in pbar:
                ...
                if len(values) != self.NUM_FEATS + 1:
                    continue
                np_array = np.zeros(self.NUM_FEATS + 1, dtype=np.int64)
