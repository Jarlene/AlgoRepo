...
class CollisionLoss(nn.Module):
        ...
        for i in range(n_futures):
            if len(future_gt_bbox[i].tensor) > 0:
                future_gt_bbox_corners = future_gt_bbox[i].corners[:, [
                    0, 3, 4, 7], :2]  # (N, 8, 3) -> (N, 4, 2) only bev
                ...
                sdc_bev_box = self.to_corners(
                    [sdc_traj_all[0, i, 0], sdc_traj_all[0, i, 1], self.w, self.h, sdc_yaw])
    ...
    def inter_bbox(self, corners_a, corners_b):
        ...
        xi1, yi1 = torch.min(xa1, xb1), torch.min(ya1, yb1)
        xi2, yi2 = torch.max(xa2, xb2), torch.max(ya2, yb2)
        intersect = torch.max((xi1 - xi2), xi1.new_zeros(1, ).to(xi1.device)) * \
            torch.max((yi1 - yi2), xi1.new_zeros(1,).to(xi1.device))
        return intersect
    ...
    def to_corners(self, bbox):
        ...
        new_corners = rot_mat @ corners.T + \
            torch.tensor(bbox[:2])[:, None].to(x.device)
