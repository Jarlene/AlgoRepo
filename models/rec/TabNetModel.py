...
import numpy as np
from models.rec.Sparsemax import Sparsemax, Entmax15
...
class TabNetEncoder(torch.nn.Module):
    ...
    def forward_masks(self, x):
        ...
        for step in range(self.n_steps):
            ...
            M_explain += torch.mul(M_feature_level,
...
class TabNetPretraining(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        if self.training:
            masked_x, obfuscated_groups, obfuscated_vars = self.masker(
                embedded_x)
...
class EmbeddingGenerator(torch.nn.Module):
    ...
    def __init__(self, input_dim, cat_dims, cat_idxs, cat_emb_dims, group_matrix):
        ...
        self.post_embed_dim = int(
            input_dim + np.sum(cat_emb_dims) - len(cat_emb_dims))
...
class RandomObfuscator(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        obfuscated_groups = torch.bernoulli(
            ...
            torch.ones((bs, self.num_groups), device=x.device)
        )
